package com.ejb.uplus.view;

import org.junit.Before;
import org.junit.Test;

/**
 * Created by John on 11/3/2016.
 */
public class HomeActivityTest
{
    @Before
    public void setUp() throws Exception
    {

    }

    @Test
    public void onCreate() throws Exception
    {

    }

    @Test
    public void getBaseView() throws Exception
    {

    }

    @Test
    public void loadStart() throws Exception
    {

    }

    @Test
    public void loadSuccess() throws Exception
    {

    }

    @Test
    public void loadError() throws Exception
    {

    }

    @Test
    public void initViews() throws Exception
    {

    }

    @Test
    public void resetTabItems() throws Exception
    {

    }

    @Test
    public void setListeners() throws Exception
    {

    }

    @Test
    public void onBackPressed() throws Exception
    {

    }

    @Test
    public void initPage() throws Exception
    {

    }

    @Test
    public void initChildren() throws Exception
    {

    }

    @Test
    public void initConfigers() throws Exception
    {

    }

    @Test
    public void resetContent() throws Exception
    {

    }

    @Test
    public void setBarTitle() throws Exception
    {

    }

    @Test
    public void onClick() throws Exception
    {

    }

    @Test
    public void onLeftClick() throws Exception
    {

    }

    @Test
    public void onRightClick() throws Exception
    {

    }

}